...
- Prioritise sharp original-paper figures, result images, and diagrams that can create an immediate “oh, I see” moment: they should make the mechanism, spatial structure, or experimental outcome legible at a glance. Treat these visuals as core teaching evidence, not decoration or an optional replacement for prose. When a source visual clearly earns this role, extract a tight high-resolution crop and place it beside the explanation it unlocks.
- Do not let a custom lesson diagram or a reconstructed table become a reason to omit a useful original-paper visual. Use both when they serve distinct jobs: the lesson diagram for the simplified mental model, and the original visual for the paper’s exact architecture, trajectory, qualitative result, or evidence.
