...
For a substantial new collection lesson, use independent research/verification, visual-source audit, and pedagogy/design briefs whenever delegation is available. Give the visual-source auditor exclusive ownership of the current lesson’s visual-asset files: it must render, inspect, and save high-resolution local PNG crops for every recommended source figure, table, or plot, with provenance and teaching role. The supervisor selects only from the returned assets and references them in the HTML; do not have the supervisor create source-image crops. Do not impose a fixed maximum number of visuals in the final lesson. Integrate every sharp, non-redundant source visual that advances the causal teaching flow, and remove unused current-run candidate assets before delivery. For smaller revisions, use delegation when the user asks or when it materially improves confidence. Integrate the evidence into one coherent output; do not paste briefs together.
